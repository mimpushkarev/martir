import {FC, memo} from 'react';

import {PriceTag} from '_entities/price-tag';
import {ViewedProducts} from '_entities/viewed-products';
import {Accordeon} from '_shared/accordeon';
import {Container} from '_shared/container';
import Slider from '_shared/slider/Slider';

const CONTENT_FOR_PRODUCT_PAGE = {
  product_photos: [
    'https://images.unsplash.com/photo-1691269578589-6b9134ca8107?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=627&q=80',
    'https://images.unsplash.com/photo-1682685797332-e678a04f8a64?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
    'https://images.unsplash.com/photo-1691269579045-09882e60bf68?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=627&q=80',
    'https://images.unsplash.com/photo-1566807387450-b74aea0e727e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
    'https://images.unsplash.com/photo-1682685797229-b2930538da47?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80',
    'https://plus.unsplash.com/premium_photo-1675804669860-9e27f22b0681?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1650&q=80',
    'https://images.unsplash.com/photo-1682685797332-e678a04f8a64?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
    'https://images.unsplash.com/photo-1691269579045-09882e60bf68?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=627&q=80',
    'https://images.unsplash.com/photo-1566807387450-b74aea0e727e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
    'https://images.unsplash.com/photo-1682685797229-b2930538da47?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80'
  ],
  product_description:
    'Жаккардовое оверсайз худи с нашивкой “Familiar look” - это уникальный товар из коллекции А-2023. Элементы наследия переосмыслены в новых принтах и деталях жаккарда. Мотив темных улиц, вдохновленный городской культурой, представлен в виде повторяющегося рисунка на этом оверсайз худи, сочетающегося с графичными линиями и абстрактными формами. Оверсайз крой и мягкий материал обеспечивают максимальный комфорт во время носки.',
  product_features: {
    name: 'Материалы и уход',
    info: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id velit varius, dignissim diam sed, condimentum tellus. Curabitur metus elit, placerat ut pulvinar in, bibendum id ex. Maecenas id felis in augue venenatis pulvinar nec id libero. Sed aliquet, est eget aliquam molestie, nulla lacus aliquam neque, ac scelerisque magna sem nec tellus. Phasellus efficitur sit amet quam id gravida. Vestibulum ultricies aliquam congue. Integer in eleifend libero, eu cursus magna. Integer mattis molestie lorem a rutrum. Ut tellus enim, vehicula sed lacus eget, rhoncus venenatis nulla. Cras neque nunc, efficitur non iaculis a, iaculis in ipsum. Vivamus ultrices lorem lorem, a fringilla sem mattis eu. Sed consequat aliquam tellus, eu suscipit metus mattis id. Ut aliquet suscipit odio, id commodo massa feugiat et. Fusce sit amet dui non nisl imperdiet tristique ac nec ipsum. Proin mollis orci sed erat convallis, in maximus tellus mollis. Ut pellentesque lacus in odio tristique finibus.'
  },
  service: [
    {
      name: 'Упаковка',
      info: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id velit varius, dignissim diam sed, condimentum tellus. Curabitur metus elit, placerat ut pulvinar in, bibendum id ex. Maecenas id felis in augue venenatis pulvinar nec id libero. Sed aliquet, est eget aliquam molestie, nulla lacus aliquam neque, ac scelerisque magna sem nec tellus. Phasellus efficitur sit amet quam id gravida. Vestibulum ultricies aliquam congue. Integer in eleifend libero, eu cursus magna. Integer mattis molestie lorem a rutrum. Ut tellus enim, vehicula sed lacus eget, rhoncus venenatis nulla. Cras neque nunc, efficitur non iaculis a, iaculis in ipsum. Vivamus ultrices lorem lorem, a fringilla sem mattis eu. Sed consequat aliquam tellus, eu suscipit metus mattis id. Ut aliquet suscipit odio, id commodo massa feugiat et. Fusce sit amet dui non nisl imperdiet tristique ac nec ipsum. Proin mollis orci sed erat convallis, in maximus tellus mollis. Ut pellentesque lacus in odio tristique finibus.'
    },
    {
      name: 'Доставка и условия возврата товара',
      info: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id velit varius, dignissim diam sed, condimentum tellus. Curabitur metus elit, placerat ut pulvinar in, bibendum id ex. Maecenas id felis in augue venenatis pulvinar nec id libero. Sed aliquet, est eget aliquam molestie, nulla lacus aliquam neque, ac scelerisque magna sem nec tellus. Phasellus efficitur sit amet quam id gravida. Vestibulum ultricies aliquam congue. Integer in eleifend libero, eu cursus magna. Integer mattis molestie lorem a rutrum. Ut tellus enim, vehicula sed lacus eget, rhoncus venenatis nulla. Cras neque nunc, efficitur non iaculis a, iaculis in ipsum. Vivamus ultrices lorem lorem, a fringilla sem mattis eu. Sed consequat aliquam tellus, eu suscipit metus mattis id. Ut aliquet suscipit odio, id commodo massa feugiat et. Fusce sit amet dui non nisl imperdiet tristique ac nec ipsum. Proin mollis orci sed erat convallis, in maximus tellus mollis. Ut pellentesque lacus in odio tristique finibus.'
    }
  ],
  viewed_products: [
    {
      image:
        'https://images.unsplash.com/photo-1691269578589-6b9134ca8107?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=627&q=80',
      link: 'https://example.com/',
      name: 'худи'
    },
    {
      image:
        'https://images.unsplash.com/photo-1682685797229-b2930538da47?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80',
      link: 'https://example.com/',
      name: 'шорты'
    },
    {
      image:
        'https://images.unsplash.com/photo-1566807387450-b74aea0e727e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
      link: 'https://example.com/',
      name: 'тишка'
    }
  ],
  product_info: {
    image:
      'https://images.unsplash.com/photo-1566807387450-b74aea0e727e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
    link: 'https://example.com/',
    name: 'Жаккардовое оверсайз худи с нашивкой “Familiar look"',
    price: '15000',
    size_guide_link: 'https://example.com/'
  },
  sales: [
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id velit varius, dignissim diam sed',
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id velit varius, dignissim diam sed',
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id velit varius, dignissim diam sed'
  ]
};

const Product = memo<FC>(function Product() {
  return (
    <Container px={6} py={6} className="flex flex-col gap-16">
      <Slider images={CONTENT_FOR_PRODUCT_PAGE.product_photos} duration={3000} className=" h-[500px]" />
      <PriceTag product={CONTENT_FOR_PRODUCT_PAGE.product_info} />
      <div className="flex w-2/3 flex-col gap-10 text-paragraph">
        <div>{CONTENT_FOR_PRODUCT_PAGE.product_description}</div>
        <div className="flex flex-col gap-6">
          <Accordeon title={CONTENT_FOR_PRODUCT_PAGE.product_features.name}>
            {CONTENT_FOR_PRODUCT_PAGE.product_features.info}
          </Accordeon>
          {CONTENT_FOR_PRODUCT_PAGE.service.map(item => (
            <Accordeon title={item.name} key={item.name}>
              {item.info}
            </Accordeon>
          ))}
        </div>
      </div>
      <ViewedProducts products={CONTENT_FOR_PRODUCT_PAGE.viewed_products} />
    </Container>
  );
});

export default Product;
